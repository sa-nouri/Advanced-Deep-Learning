...
class TextClassificationTrainer:
    def __init__(self, model, device, model_type='bert', learning_rate=2e-5, weight_decay=0.01):
        self.model = model
        self.device = device
        self.model_type = model_type
        self.criterion = nn.CrossEntropyLoss()
        
        # Different optimizers for different model types
        if model_type == 'bert':
    ...
    def train(self, train_loader, val_loader, num_epochs, save_dir, warmup_steps=0):
        ...
        try:
            # Create scheduler (only for BERT)
            if self.model_type == 'bert':
