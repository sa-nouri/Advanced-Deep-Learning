...
import ops
from tensorflow.python.keras.layers import Dense, Conv2D, BatchNormalization, Conv2DTranspose, Flatten, LeakyReLU
...
class infoGAN9(tf.keras.models):
    
    def __init__(self, config) -> None:
        self.conv = Conv2D(1024, 1, 1, kernel_initializer=config.kernel_initializer)
        self.conv_discriminator = Conv2D(128, 10, 1)
        self.leaky_relu = LeakyReLU(alpha=0.2)

    def __call__(self, x):
        x = self.leaky_relu(tf.nn.softmax(self.conv(x)))

        discriminator = self.discriminator(x)
        return discriminator.squeeze()